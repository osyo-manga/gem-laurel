...
describe Laurel::Proxy do
	...
	describe ".proxy_all" do
		subject {
			Laurel.proxy_all do |name, *args, &block|
				{
					name: name,
					args: args,
					block: block
		...
		it { expect(subject != 10).to  include(name: :!=,  args: [10]) }
		it { expect(!subject).to       include(name: :!,   args: []) }
		it { expect(subject.__id__).to include(name: :__id__, args: []) }
		it { expect(subject.__send__ :homu, 1, 2).to include(name: :__send__, args: [:homu, 1, 2]) }
	end
end
